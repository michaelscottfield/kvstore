...
int FileIo::Append(KVString &key, KVString &val){
    ...
    //LOG(ERROR)<<"this function has been used";
    //LOG(INFO)<<"this function has been used";
    //LOG(INFO)<<key<<"----"<<val;
    //LOG(INFO)<<key.Buf();
    ...
    if(fwrite(val.Buf(),val_size,1,fh_)!=1){
        LOG(ERROR)<<"write val buf failed: "<<errno<<"-"<<strerror(errno);
...
int FileIo::ReadKV(int offset, KVString &key, KVString &val) {
    ...
    char buf[sizeof(int) * 2];//sizeof(int)*2=8
    //LOG(ERROR)<<sizeof(int)*2;
    ...
    //if(val_size == 0) LOG(ERROR)<<"cannot be";
    //LOG(ERROR) << sizeof(buf) << "---" << sizeof(buf+sizeof(int)); 
    //LOG(ERROR)<<strlen(buf)<<"---"<<strlen(buf + sizeof(int));
    //if(strlen(buf+sizeof(int)) < strlen(buf))   LOG(ERROR) << "impossible";
    //LOG(ERROR)<<buf[7];
    //LOG(ERROR)<<key_size<<"---"<<val_size;
    //LOG(ERROR)<<val_size;
